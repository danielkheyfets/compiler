...
;; sets the car of %1 pair sob to be %2 sob 
%macro SET_CAR 2
	lea rbx, [%1+TYPE_SIZE]
	mov [rbx], %2
%endmacro
;; sets the cdr of %1 pair sob to be %2 sob
%macro SET_CDR 2
	lea rbx, [%1+TYPE_SIZE+WORD_SIZE]
	mov qword [rbx], %2
...
%define PVAR(n) qword [rbp+WORD_SIZE*(4+n)]
%define PARAM_COUNT qword [rbp+WORD_SIZE*3]
%define ENV_ON_STACK qword [rbp+WORD_SIZE*2]
%define OLD_RETURN_ADDRESS qword [rbp+WORD_SIZE*1]
%define ENV(env, i) qword [env+WORD_SIZE*i]
%define VECTOR(arr, i) qword [arr+WORD_SIZE*i]
;; set param count on stack to value of %1
%macro SET_PARAM_COUNT 1
	mov [rbp+3*WORD_SIZE], %1
%endmacro
...
%define MAKE_LITERAL_CHAR(ascii_val) \
		MAKE_LITERAL T_CHAR, db ascii_val
...
%define MAKE_LITERAL_SYMBOL(sob_ptr) \
		MAKE_LITERAL T_SYMBOL, dq sob_ptr
...
;; %1 dest reg to put the result , %2 pointer to SOB_PAIR
%macro LIST_LENGTH 2
	push %2
	push r8
	mov r8, 0
...
;; %1 SOB last param ,  
%macro GET_LAST_PARAM 1
	push r12
	mov r12, PARAM_COUNT
	sub r12, 2 ;;minus proc as arg and magic
	mov %1, PVAR(r12)
	pop r12
%endmacro
