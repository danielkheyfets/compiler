...
module Prims : PRIMS = struct
  ...
  let my_add =
    ...
    String.concat "\n\n" (List.map (fun (a, b, c) -> (b c a)) ops);;
let apply = 
"
apply:
    push rbp
    mov rbp, rsp 
    GET_LAST_PARAM rcx                          ;; rcx has the pointer to SOB Pair
    LIST_LENGTH rbx, rcx                        ;; rbx contains length of list
    mov r11, rbx                                ;; r11 = length of list => save the length of list for later
    push SOB_NIL_ADDRESS                        ;; push MAGIC
    lea rbx, [rbx*WORD_SIZE]
    sub rsp, rbx                               ;; make room for copying the elements in stack            
    mov rax, rsp
    .copy_lst_loop:
      cmp rcx, SOB_NIL_ADDRESS
      je .end_copy_lst
      CAR r8, rcx                            ;; car of list is in r8
      mov qword [rax], r8                    ;; add to stack
      add rax, WORD_SIZE                     ;; point to next reserved place in stack
      CDR rcx, rcx                           ;; rcx = CDR(rcx)
      jmp .copy_lst_loop
    .end_copy_lst:
      ;; by now all elements of list are in the stack and rsp is in the right place
      ;; now copy rest args
      mov r12, PARAM_COUNT
      mov r8, r12                            ;; save for shift rsp later
      sub r12, 3                             ;; becuase of the list i took care , no magic, no proc
      add r11 , r12                          ;; r11 = number of list elements + rest args
    .copy_rest_args:
      cmp r12, 0
      je .end_copy_rest_args
      push PVAR(r12)
      dec r12
      jmp .copy_rest_args
    .end_copy_rest_args:
      ;; at this point all args for proc are in stack

      mov rbx, PVAR(0)                      ;; closure of proc
      CLOSURE_ENV rcx, rbx                  ;; rcx = Env of proc
      CLOSURE_CODE rax, rbx                 ;; rax = Code of proc
	    inc r11                               ;; magic is part of the arg count
      
      push r11                               ;; push args_count
      push rcx                               ;; push env
      push OLD_RETURN_ADDRESS                ;; push ret address
      push qword [rbp]                       ;; push old rbp
      
      add r11, 4                             ;; new frame size = args + old rbp + ret + env proc
      MY_SHIFT_FRAME r11
      pop rbp
      lea rsp, [rsp+WORD_SIZE*(r8+4)]      ;; update the rsp
  ...
  let procs = String.concat "\n\n" [type_queries ; numeric_ops; misc_ops;my_add]^apply;;
end;;
